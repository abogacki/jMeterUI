...
const calcStats = groupedData => {
  ...
  const stats = Object.keys(transformedGroups).map(groupName => ({
    ...
    median:
      Math.round(statsLite.median(transformedGroups[groupName]) * 100) / 100,
    mode: statsLite.mode(transformedGroups[groupName]),
    variance:
      Math.round(statsLite.variance(transformedGroups[groupName]) * 100) / 100,
    standardDeviation:
      Math.round(statsLite.stdev(transformedGroups[groupName]) * 100) / 100,
    sampleStandardDeviation:
      Math.round(statsLite.sampleStdev(transformedGroups[groupName]) * 100) /
      100,
    percentile:
      Math.round(
        statsLite.percentile(transformedGroups[groupName], 0.85) * 100
      ) / 100,
    histogram:
      Math.round(statsLite.histogram(transformedGroups[groupName], 10) * 100) /
      100
  }));
